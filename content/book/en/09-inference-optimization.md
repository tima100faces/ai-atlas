...
Because of this, inference optimization is an interdisciplinary field that
...
inference workloads it serves. There are two main computational
...
TERMINOLOGY AMBIGUITY: MEMORY-BOUND VERSUS BANDWIDTH-
BOUND
...
compute-bound or memory bandwidth-bound, as shown in Figure 9-2. This
...
Figure 9-3. Autoregressive language models follow two steps for inference: prefill and decode.
...
The total latency will equal TTFT + TPOT × (number of output
tokens).
...
Throughput is directly linked to compute cost. A higher throughput
...
throughput but reduce latency. According to the LinkedIn AI team in their
...
Figure 9-4. If an inference service can complete 10 RPS but only 3 satisfy the SLO, then its goodput
is 3 RPS.
...
which represents the percentage of time during which the GPU is actively
...
Table 9-1. MFU examples from “PaLM: Scaling Language Modeling with Pathways” (Chowdhery et
al., 2022).
Model
Number of
parameters (in
...
Figure 9-5. Bandwidth utilization for Llama 2-70B in FP16 across three different chips shows a
...
understanding hardware allows for deeper optimization. This section looks
...
HBM provides significantly higher bandwidth, with data transfer
...
Figure 9-7. The memory hierarchy of an AI accelerator. The numbers are for reference only. The
...
billions of transistors—an NVIDIA A100 has 54 billion transistors, while an
...
If you opt for cloud providers, you won’t need to worry about cooling or
...
Since hardware design is outside the scope of this book, I’ll discuss
...
Model distillation suggests that it’s possible to capture a large model’s
...
original model’s architecture, and the performance boost it can bring is
...
As the space is rapidly evolving, new techniques are being developed to
...
conditioned on x , x , …, x , x
, x
, …, x
. The process is visualized
in Figure 9-9.
2
t
t + 1
...
50 lines of code in PyTorch. It’s been incorporated into popular inference
...
Lossless Acceleration of Large Language Models” (Yang et al., 2023), this
...
Figure 9-10. Two examples of inference with reference. The text spans that are successfully copied
...
, the k head will predict the token
x
...
Figure 9-11. In Medusa (Cai et al., 2024), each head predicts several options for a token position. The
...
, instead of computing the key and value
vectors for tokens x , x , …, x
 again, you reuse these vectors from the
...
Grouped-query attention (Ainslie et al., 2023) is a generalization of multi-
...
dividing the KV cache into non-contiguous blocks, reducing fragmentation,
...
Figure 9-13. FlashAttention is a kernel that fuses together several common operators. Adapted from
...
Common AI operations, including matrix multiplication, attention
...
kernels. There are many great online tutorials for kernel writing. Here, I’ll
...
While vectorization, parallelization, and loop tiling can be applied
...
frameworks, like torch.compile (a feature in PyTorch), XLA
...
people, but it can also make each person’s journey longer. However, if you
...
Figure 9-16. With continuous batching, completed responses can be returned immediately to users,
...
prefilling job can drain computational resources from existing decoding
...
Figure 9-17. With a prompt cache, overlapping segments in different prompts can be cached and
reused.
...
to maximize your metrics. For example, should you place three 13B
...
influenced by the prefilling phase, and time per output token (TPOT),
...
However, across various use cases, the most impactful techniques are
