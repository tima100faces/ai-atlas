...
through various retrieval mechanisms, including text retrieval, image
...
Leaking private information to external APIs is a risk specific to using
...
Recall from Chapter 5 that for security measurements, it’s important to
...
users’ messages. Another team transfers a conversation after a certain
...
Given the many different places where an application might fail, guardrails
...
Given that gateways are relatively straightforward to implement, there are
...
This approach requires a vector database to store the embeddings of cached
queries.
...
Model API box. There’s a new arrow to add generated responses to the
cache.
...
perform another retrieval to gather more information. The original response,
...
If you’ve followed all the steps so far, your architecture has likely grown
...
Observability is a universal practice across all software engineering
...
Having a high CFR doesn’t necessarily indicate a bad monitoring system.
...
The easiest types of failures to track are format failures because they are
...
What’s the average number of tokens per output? Are some models more
...
When computing metrics, ensure they can be broken down by relevant axes,
...
For fast detection, metrics need to be computed quickly. For fast response,
...
interact with more data, allowing them to both rewrite their prompts to
...
be obvious what caused this gradual drop. You need investigations to
...
You need to tell the orchestrator what components your system uses,
...
step is in the format expected by the next step. Ideally, it should notify you
...
As you advance to the later stages of your application development process,
...
team. Avoid orchestrators that initiate hidden API calls or introduce
...
responds with three recommendations as follows:
...
nothing under $200?” reveals a price-conscious preference and suggests
...
(Ponnusamy et al., 2019; Park et al., 2020), Spotify’s voice control feature
...
misunderstanding. Rephrase attempts can be detected using heuristics or
ML models.
...
Table 10-1. Feedback types derived from automatic clustering the FITS dataset (Xu et al., 2022). Resu
al. (2023).
Group
Feedback type
Num.
...
26
Complain that the bot (1) does not
answer the question or (2) gives
irrelevant information or (3) asks
the user to find out the answer on
their own.
16
Point out specific search results
that can answer the question.
16
Suggest that the bot should use the
search results.
15
State that the answer is (1)
factually incorrect, or (2) not
grounded in the search results.
11
Point out that the bot’s answer is
not
...
someone starts a conversation angry but ends happily, the conversation
...
Personally, I often choose regeneration for complex requests to ensure the
...
coworkers. It’s important to study your users to understand why they do
each action.
...
model can output both summaries side by side, assuming that generating
...
Figure 10-17. Google Photos asks for user feedback when unsure. The two cat images were
...
However, many people I’ve talked to believe users should have the option
...
Figure 10-20. An example of ChatGPT asking a user to select the response the user prefers. However,
...
Figure 10-21. Because Luma put the angry emoji, corresponding to a one-star rating, where a five-
...
follow, hiding likes could result in users’ confusion about why certain
...
On a five-star rating scale, four and five stars are typically meant to
...
cat photos. The system picks up on this and starts generating more photos
...
Hacked” (Liam Niemeyer, WKU Public Radio, March 2023).
