...
It begins with data curation, addressing questions like What data do you
...
In 2023, DataComp (Gadre et al., 2023) hosted a competition whose goal
...
work closely with application and model developers. In a small team, they
...
containing one message. However, for tool use, the AI might need to
...
bad behaviors. Imagine you work on a chatbot like ChatGPT and you hear
...
aligned with task requirements, consistent, correctly formatted, unique, and
...
Annotations should be consistent across examples and annotators. If
...
Different applications have different dimensions of diversity. For example,
...
finetuning, and preference finetuning. While this chapter focuses on post-
...
Figure 8-1. A 7B-parameter model, finetuned on a dataset that is both high-quality and diverse,
...
tokens, respectively. If each example is 2,000 tokens, it’d be equivalent to 1
...
such as a question answering about financial filings.
...
Figure 8-2. With 100 examples, more advanced models give much better performance after
...
However, be careful before concluding that finetuning with a small dataset
...
doubling your data. A plateau slope means that doubling your data will give
...
Instruction-Finetuned Language Models” (Chung et al., 2022) shows that
...
Figure 8-4. Diversity in finetuning number, measured by the number of tasks, can impact model
...
multiple data sources via multiple acquisition channels. For example, the
...
might find that having 100 synthetic instructions per template hurts your
...
9. Some evaluation harness tools host evaluation benchmark datasets that
...
should invest more time in curating evaluation guidelines and data. If
...
Artificially generated data has a long history in software engineering. It was
...
The biggest reason for data synthesis is that it allows you to produce
...
political views, ethical stances, and social biases. They found that in
...
also by mood and motivations. AI-generated preference ratings, in
...
10 test dataset and 16.04% of the ImageNet test images could be
...
Simulations are common to generate data to teach models to use tools. As
...
which one works better. You can have one version of the model play the
...
stories, posts, and WikiHow articles generated by Mixtral-8x7B-Instruct-
...
For instruction generation, to ensure that you generate sufficient
...
including Variational Autoencoders, Gaussian mixture models, and LLMs.
...
data during its instruction finetuning and preference finetuning phase.
...
Alpaca, behaves similarly to text-davinci-003, while being 4% the size of
the teacher model.
NOTE
...
(Jiang et al., 2024), a 56-billion-parameter mixture-of-experts model. The
...
You can be creative in the statistics to use to understand your data. For
...
GPT-4 seems to have a broader and more diverse range of verb-noun
...
annotations match the given annotations. This will give you a sense of how
...
Output
(Price)
{item: pencil, color: re
d}
$20
{item: compass, color: gr
een}
$2
{item: pencil, color: re
d}
...
don’t hurt your model’s performance, they can waste your time and
compute.
...
Label the following item as either edible or
inedible.
Item: burger
Label: edible
Item: car
...
uses the prompt in the format “burger -->”, any of the following prompts
can cause issues:
...
many teams have found that increasing the diversity of their datasets is key
...
4
 ...
 While this doesn’t refer to the uniqueness of your data, having data that nobody else has can be
extremely valuable.
...
4
 ...
 Comparing the parameter count of a mixture-of-experts model like Mixtral to that of a dense model
